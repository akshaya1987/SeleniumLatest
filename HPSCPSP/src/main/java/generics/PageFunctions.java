...
//    static WebDriver driver = DriverFactory.getDriver();
    ...
    public static void _clearAndSendKeys(By by, String text, String eleName) {
        ...
        try {
            webElement = _performExplicitWait(by, CLICKABLE);
//            String os = System.getProperty("os.name");
    ...
    public static void _sendKeys(By by, String text, String eleName) {
        WebElement webElement;
        try {
            webElement = _performExplicitWait(by, CLICKABLE);
//            String os = System.getProperty("os.name");
...
//        if (!elementName.isEmpty()) Logger.info("Clicked on " + elementName);
    ...
    public static void _scrollIntoViewAndClick(By by, String elementName) {
        WebElement e = _performExplicitWait(by, PRESENCE);
...
//        if (!elementName.isEmpty()) Logger.info("Clicked on " + elementName);
    ...
    public static void _sendKeysByJavascriptExecutor(By by, String elementName, String keytosend) {
        WebElement e = _performExplicitWait(by, VISIBLE);

        ((JavascriptExecutor) DriverFactory.getDriver()).executeScript("arguments[0].value='" + keytosend + "';", e);
//        if (!elementName.isEmpty()) Logger.info("Clicked on " + elementName);
        e.sendKeys(Keys.ENTER);
    }

    public static void enterStringByCharacter(By by, String elementName, String keytosend){
        WebElement e = _performExplicitWait(by, VISIBLE);
        for (int i=0; i<keytosend.length(); i++) {
//            ((JavascriptExecutor) DriverFactory.getDriver()).executeScript("arguments[0].value='" +keytosend.charAt(i) + "';", e);
            String s = new StringBuilder().append(keytosend.charAt(i)).toString();
            e.sendKeys(s);
        }
        e.sendKeys(Keys.ENTER);
    }

    public static boolean _isElementAvailable(By by, WaitStrategy waitStrategy) {
    ...
    public static void _scrollIntoView(By by, WaitStrategy waitStrategy){
        WebElement element = _performExplicitWait(by, waitStrategy);
        JavascriptExecutor jse = (JavascriptExecutor) DriverFactory.getDriver();
